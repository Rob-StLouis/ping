...
Participants DNA was collected with a mouth swab and a whole genotype sequencing was performed using the Illumina Human660W-Quad BeadChip. Included in the 494,082 SNPs passing the PING quality control filters were the Val158Met polymorphism that influences the COMT gene (rs4680), and an SNP influencing post-synaptic D2 density (rs12364283) in the striatum (Frank & Hutchingson, 2011). For each participant, we assigned a scalar gene-dose value to them, definite in terms of the likely protective effect of their alleles. For COMT, MET/MET =2, MET/VAL =0, VAL/VAL =-1, their relative proportions are 14:81:56 respectively. For rs12364283, AA= 1, GA = 0 with relative proportions of 141:10 respectively. We referred to this SNP as DRD2 for simplicity later in the analysis. Other studies have found a population frequency of roughly 15% for the GA allelic variety [@frank2009genetic], so the minority variant is significantly underrepresented in our population (Binomial test(exact) p = .002). The distribution of the rs12364283 and rs4680 allels appear to be independent $\chi^2$=0.232, p=.896.     
...
We conducted a logistic regression, the beta weights and Wald-z values are reported blow. As is clear, nothing except age is signifcant at the $\alpha$=.05 level. There is no evidence that the COMT polymorphism is related to substance use. 


**Logistic Regression on Substance Use** 
...
**Likelihood Ratio Test on the interaction between COMT and Income On Substance Use **
```{r,echo=FALSE,results='asis'}
...
**Logistic Regression of DRD2 on Substance Use **
```{r,echo=FALSE,results='asis'}
...
qplot(Household_Income,UseSubstance,geom=c("smooth"),method="glm",data=DataOut[DataOut$Log10_Household_Income>4.5,],facets=.~rs12364283)+geom_point(aes(Household_Income,jitter(UseSubstance)))+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+scale_x_continuous(labels = comma) +xlab("Household Income")+ ylab("Used a Substance (1 = Yes)" )+ ggtitle("Figure2b: \n Influence of rs12364283 Alleles \n and Income on Substance Use, \n Income>$30,000")
...
**Likelihood Ratio Test of the Interaction between DRD2 and Income on Substance Use for Incomes > $30,000 and with the Outlier Dropped**  
...
m5<-glm(UseSubstance~Age+DRD2*Log10_Household_Income,family='binomial',data=DataOut[DataOut$Log10_Household_Income>4.5,])
